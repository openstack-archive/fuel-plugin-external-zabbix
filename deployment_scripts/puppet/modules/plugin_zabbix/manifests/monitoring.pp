...
class plugin_zabbix::monitoring(
  ...
  $zabbix_agent_support = member($myroles, 'controller') or member($myroles, 'primary-controller') or member($myroles, 'ceph-osd') or member($myroles, 'cinder') or member($myroles, 'compute')

  if ! $zabbix_agent_support {
