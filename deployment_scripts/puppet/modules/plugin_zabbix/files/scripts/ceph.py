...
class CephMonPlugin(base.ZabbixSender):
    ...
    def send(self):
        for m in plugin.itermetrics_status():
            self.zabbix_sender(m['type'], m['value'])
...
# The following code avoids to collect metrics from all controllers.
# If ceph-mon or zabbix-server are no more co-localized (on controllers) the
# script will be broken (will report nothing)
# This is currently acceptable since there are no plan to 'detach' either
# ceph-mon or zabbix-server
out, err = plugin.execute('/usr/bin/sudo /usr/sbin/crm_resource --locate --quiet --resource p_zabbix-server')
if not out or err or out != socket.gethostname():
    print 0
    sys.exit(0)
