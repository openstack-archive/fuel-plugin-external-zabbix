...
* :ref:`tunning` for server and agents
...
* Compatibility with MOS 7.0 *(follow-up)*
  ...
  * Monitor `cinder-volume` process (instead of the Pacemaker resource which has
    been removed)
  * Fix trigger for Neutron DHCP/L3 agent (these agents run now on all controllers)
...
* Prior to version 2.5.0, the plugin requires access to distribution repository,
  ...
  installation.
  With plugin version 2.5.0, the `fuel-createmirror` command is supported.
